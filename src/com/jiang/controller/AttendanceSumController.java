...
import com.jiang.service.Impl.AttendanceSumBizImpl;
@WebServlet("/AttendanceSumController/*")
public class AttendanceSumController extends HttpServlet {
	private AttendanceSumBiz  biz = new AttendanceSumBizImpl();
	/**
	 * Constructor of the object.
	 */
	public AttendanceSumController() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		switch(opt)
		{
		case "QueryAllAttendance":
			QueryAllAttendance(request,response);
			break;
		}
	}

	private void QueryAllAttendance(HttpServletRequest request,
		...
		if("true".equals(radiotf)){
			...
			}else
			{
				...
				try {
					...
					employee.setAttendanceDate(attendanceDate);
		...
		if(request.getParameter("attendanceType") != null && !request.getParameter("attendanceType").equals(""))
		{
			
			String attendanceType = request.getParameter("attendanceType");
			employee.setAttendanceType(attendanceType);
		}
		List<Employee>  list = biz.findAll(employee);
